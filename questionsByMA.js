...
const fn = ({ videos }) => videos.reduce((acc, curr, i) => {

  const comment = curr.comments.map(comt => comt.comment);
  //   console.log(comment)
  acc = [...acc, { [curr.id]: comment }]
  return acc;
}, []);
console.log(fn(videoJSON))
...
const numbers6 = [10, 2, 7, 14, 5];

const secondLargest = (numbers) => {
  if (numbers.length < 2) return "less than 2";
  return numbers.reduce(
    (acc, curr) => {
      if (curr > acc.largest) {
        acc.second = acc.largest;
        acc.largest = curr;
      } else if (curr > acc.second) acc.second = curr;
      return acc;
    },
    { largest: 0, second: 0 }
  );
...
const totalAmt = orders => orders.reduce((acc, { items }) => acc + items.length * 10, 0);
console.log(totalAmt(orders)); //90
...
const totalRevenue = sales => sales.reduce((acc, { amount, price }) => {
  return acc + amount * price
}, 0)

console.log(totalRevenue(sales)); //585
